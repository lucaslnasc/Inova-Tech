...
public class EnrollmentService {
  ...
  public Enrollment createEnrollment(UUID userId, UUID eventId) {
    ...
    // Validar se evento já não passou
    if (event.getStartDateTime().isBefore(LocalDateTime.now())) {
      throw new RuntimeException("Não é possível se inscrever em evento que já iniciou");
    }
  ...
  public void cancelEnrollment(UUID enrollmentId) {
    ...
    // Verificar se evento já não passou
    if (enrollment.getEvent().getStartDateTime().isBefore(LocalDateTime.now())) {
      throw new RuntimeException("Não é possível cancelar inscrição de evento que já iniciou");
    }
